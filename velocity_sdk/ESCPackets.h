...
void encodeESC_CommandMultipleESCsPacket(void* pkt, uint16_t pwmValueA, uint16_t pwmValueB, uint16_t pwmValueC, uint16_t pwmValueD);
...
//! return the packet ID for the ESC_CommandMultipleESCs packet
#define getESC_CommandMultipleESCsPacketID() (PKT_ESC_SETPOINT_1)
...
typedef struct
{
    uint8_t  settingsLocked; //!< Set if the ESC settings are locked
...
void encodeESC_EEPROMSettingsPacket(void* pkt, uint8_t settingsLocked, uint8_t version, uint16_t size, uint16_t checksum);

//! Decode the ESC_EEPROMSettings packet to parameters
int decodeESC_EEPROMSettingsPacket(const void* pkt, uint8_t* settingsLocked, uint8_t* version, uint16_t* size, uint16_t* checksum);
