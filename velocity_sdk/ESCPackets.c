 ...
 */
void encodeESC_CommandMultipleESCsPacket(void* _pg_pkt, uint16_t pwmValueA, uint16_t pwmValueB, uint16_t pwmValueC, uint16_t pwmValueD)
{
    uint8_t* _pg_data = getESCVelocityPacketData(_pg_pkt);
    int _pg_byteindex = 0;
    ...
    // complete the process of creating the packet
    finishESCVelocityPacket(_pg_pkt, _pg_byteindex, getESC_CommandMultipleESCsPacketID());
...
int decodeESC_CommandMultipleESCsPacket(const void* _pg_pkt, uint16_t* pwmValueA, uint16_t* pwmValueB, uint16_t* pwmValueC, uint16_t* pwmValueD)
{
    ...
    // Verify the packet identifier
    if(getESCVelocityPacketID(_pg_pkt) != getESC_CommandMultipleESCsPacketID())
        return 0;
 ...
 */
void encodeESC_EEPROMSettingsPacket(void* _pg_pkt, uint8_t settingsLocked, uint8_t version, uint16_t size, uint16_t checksum)
{
    uint8_t* _pg_data = getESCVelocityPacketData(_pg_pkt);
    int _pg_byteindex = 0;

    // Set if the ESC settings are locked
    _pg_data[_pg_byteindex] = (uint8_t)limitMax(settingsLocked, 1) << 7;

    // Version of EEPROM data
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(version, 1) << 6;
    _pg_byteindex += 1; // close bit field
 ...
 * \return 0 is returned if the packet ID or size is wrong, else 1
 */
int decodeESC_EEPROMSettingsPacket(const void* _pg_pkt, uint8_t* settingsLocked, uint8_t* version, uint16_t* size, uint16_t* checksum)
{
    int _pg_byteindex = 0;
    const uint8_t* _pg_data = getESCVelocityPacketDataConst(_pg_pkt);
    int _pg_numbytes = getESCVelocityPacketSize(_pg_pkt);
    ...
    // Set if the ESC settings are locked
    (*settingsLocked) = (_pg_data[_pg_byteindex] >> 7);

    // Version of EEPROM data
    (*version) = ((_pg_data[_pg_byteindex] >> 6) & 0x1);
    _pg_byteindex += 1; // close bit field
