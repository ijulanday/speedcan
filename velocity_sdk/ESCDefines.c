...
void encodeESC_WarningBits_t(uint8_t* _pg_data, int* _pg_bytecount, const ESC_WarningBits_t* _pg_user)
{
    ...
    _pg_data[_pg_byteindex] = (uint8_t)((0 == true) ? 1 : 0) << 7;
...
int decodeESC_WarningBits_t(const uint8_t* _pg_data, int* _pg_bytecount, ESC_WarningBits_t* _pg_user)
{
    ...
    // Reserved for future use
    _pg_user->reserved = ((_pg_data[_pg_byteindex] >> 7)) ? true : false;
...
void encodeESC_ErrorBits_t(uint8_t* _pg_data, int* _pg_bytecount, const ESC_ErrorBits_t* _pg_user)
{
    ...
    _pg_data[_pg_byteindex] = (uint8_t)limitMax(_pg_user->failedStart, 1) << 7;

    // Lost commutation
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->commutation, 1) << 6;

    // Set if hall sensor error detected
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->hallSensor, 1) << 5;

    // Current exceeded hard-limit
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->overcurrent, 1) << 4;

    // Temperature exceeded hard-limit
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->overtemperature, 1) << 3;

    // Motor commutation speed exceeded hard-limit
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->overspeed, 1) << 2;

    // Motor stopped due to high demag angle
    _pg_data[_pg_byteindex] |= (uint8_t)limitMax(_pg_user->demag, 1) << 1;
...
int decodeESC_ErrorBits_t(const uint8_t* _pg_data, int* _pg_bytecount, ESC_ErrorBits_t* _pg_user)
{
    ...
    // Set if the ESC failed to start the motor
    _pg_user->failedStart = (_pg_data[_pg_byteindex] >> 7);

    // Lost commutation
    _pg_user->commutation = ((_pg_data[_pg_byteindex] >> 6) & 0x1);

    // Set if hall sensor error detected
    _pg_user->hallSensor = ((_pg_data[_pg_byteindex] >> 5) & 0x1);

    // Current exceeded hard-limit
    _pg_user->overcurrent = ((_pg_data[_pg_byteindex] >> 4) & 0x1);

    // Temperature exceeded hard-limit
    _pg_user->overtemperature = ((_pg_data[_pg_byteindex] >> 3) & 0x1);

    // Motor commutation speed exceeded hard-limit
    _pg_user->overspeed = ((_pg_data[_pg_byteindex] >> 2) & 0x1);

    // Motor stopped due to high demag angle
    _pg_user->demag = ((_pg_data[_pg_byteindex] >> 1) & 0x1);
    _pg_byteindex += 1; // close bit field
